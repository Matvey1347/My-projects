...
let homeSocial = document.querySelector('.home__social');
video.play();
video.addEventListener('timeupdate', function() {
        if (video.duration - video.currentTime <= 0.4) {
			// video.pause()
            // Удалить обработчик события, чтобы этот блок кода выполнялся только один раз
            video.removeEventListener('timeupdate', arguments.callee);

           homeSocial.classList.add('_active')
        }
    });
 video.addEventListener('ended', function() {
        // Your code to be executed after the video finishes playing
       
        // Add your code here
    });
})

let homeBtn = document.querySelector('.home__ca-btn');

homeBtn.addEventListener('click', function(){
navigator.clipboard.writeText(document.querySelector('.home__ca-contract').textContent.replace('CA:', ''))
